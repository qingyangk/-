<template>
  <div id="Communicationmap">
    ...
    <div id="parameter">
      <div class="setup">
        <div class="ding">参数设置</div>
        <div class="di">
          <ul>
            <li>
              <div class="Sleft">空间容差：</div>
              <div class="Sright">
                <el-input-number
                  v-model="fanwei"
                  :step="100"
        ...
        <div class="xuanz" @click="shengcheng()">关系图谱生成</div>
      </div>
    </div>
    <!-- 个人轨迹表 -->
    <div id="PTTable">
    ...
    <div v-show="relationShow" id="relationShip">
      ...
      <div class="close" @click="relationclose()">
        <i class="el-icon-close" />
      </div>
      <div class="body">
        <el-button class="btns" type="primary" @click="addrelatonship"
          >添加关系</el-button
        >
        <div id="addrelationship">
          <table>
            ...
            <tr>
              <th>
                <el-button type="primary" @click="backrelatonship"
                  >取消</el-button
                >
              </th>
              <th>
                <el-button type="primary" @click="addxiansuo"
                  >添加线索</el-button
                >
              </th>
            </tr>
    ...
    <div id="right">
      ...
      <div class="top">
        <div class="lable" style="font-size: 14px">查询时间：</div>
        <div class="fTime">
          <i
      ...
      <div class="bottom">
        ...
        <div class="choice" @click="LStroke()">
          <span
            ><icon-svg
              icon-class="icon-huizhiguiji"
              style="font-size: 18px; margin-right: 10px; margin-top: 15px"
            />
            加载行程</span
          >
        </div>
...
export default {
  ...
  methods: {
    ...
    int() {
      ...
      that.scene.addImage("star", "/img/star.png");
      that.scene.addImage("end", "/img/end.png");
      that.scene.addImage("after", "/img/after.png");
      that.scene.addImage("mi", "/img/mi.png");
    ...
    LStroke() {
      if (this.json.table === "") {
        ...
        that.$store
          ...
          .then((datas) => {
            that.$store
              .dispatch("communication/CSPTrajectory", data)
              .then((dataz) => {
                document.getElementById("bg").style.display = "none";
    ...
    oneline(data) {
      ...
      if (data.length > 1) {
        ...
        that.pointAll.push({
          x: data[data.length - 1].经度,
          y: data[data.length - 1].纬度,
          icon: "end",
          id: data[data.length - 1].编号,
          地点: data[data.length - 1].地点,
          name: data[data.length - 1].姓名,
          time: data[data.length - 1].时间,
          img: data[data.length - 1].头像,
        });
      } else {
        that.map.flyTo({
      ...
      fetch(
        "https://restapi.amap.com/v3/direction/driving?origin=" +
          ...
          waypoints +
          "&destination=" +
          data[data.length - 1].经度 +
          "," +
          data[data.length - 1].纬度 +
          "&output=JSON&key=49b3d756d1c570298d8b75eedf308f88"
      )
        .then((res) => res.json())
        .then((datas) => {
          var path = "";
          if (datas.status === "1") {
            for (
              let index = 0;
              index < datas.route.paths[0].steps.length;
              index++
            ) {
        ...
        if (element.length >= 2) {
          ...
          fetch(
            "https://restapi.amap.com/v3/direction/driving?origin=" +
              ...
              waypoints +
              "&destination=" +
              element[element.length - 1].经度 +
              "," +
              element[element.length - 1].纬度 +
              "&output=JSON&key=49b3d756d1c570298d8b75eedf308f88"
          )
            .then((res) => res.json())
            .then((data) => {
              ...
              if (data.status === "1") {
                for (
                  let index = 0;
                  index < data.route.paths[0].steps.length;
                  index++
                ) {
              ...
              if (leng === 0) {
                setTimeout(() => {
                  ...
                  if (this.otherlineLayer === null) {
                    ...
                    that.legend.onAdd = function () {
                      const el =
                        document.getElementsByClassName("infolegend legend")[0];
                      el.innerHTML = "";
                      // el.innerHTML +=
                      const grades = name1;
                      const color = color1;
    ...
    Complex(user, value) {
      ...
      for (let i = 0; i < value.length; i++) {
        ...
        data.timz = that.checkTime2(
          new Date(
            c.setMinutes(
              new Date(value[i].时间).getMinutes() - (that.jiange * 1) / 2
            )
          )
        );
        data.timx = that.checkTime2(
          new Date(
            c.setMinutes(
              new Date(value[i].时间).getMinutes() + (that.jiange * 1) / 2
            )
      ...
      if (this.form.relation === "") {
        ...
        var __graph_json_data = {
          nodes: [
            {
              id: _new_node_id,
              name: _new_node_name + _index,
              innerHTML:
                '<div class="c-my-node" style="background-image: url(https://wx2.sinaimg.cn/mw600/006qkfXVly1g8t75zyqkvj30u00uc42f.jpg);border:#ff875e solid 3px;"><div class="c-node-name" style="color:#ff875e">' +
                _new_node_name +
                "</div></div>",
            },
          ],
          links: [{ from: _old_form, to: _new_node_id }],
        };
